...
import { salesData, mockProducts, mockCustomers } from '../data/mockData';

const COLORS = ['#0088FE', '#00C49F', '#FFBB28', '#FF8042', '#8884d8'];

const Reports: React.FC = () => {

    const productCategories = mockProducts.reduce((acc, product) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white p-5 rounded-xl shadow-sm">
                 ...
                 <p className="text-slate-600">Valor total del inventario actual: <span className="font-bold text-xl text-green-600">$ {mockProducts.reduce((acc, p) => acc + (p.costPrice * p.stock), 0).toFixed(2)}</span></p>
            </div>
             <div className="bg-white p-5 rounded-xl shadow-sm">
                 <h2 className="text-lg font-semibold text-slate-800 mb-4">Ranking de Clientes</h2>
                 <ul className="space-y-2">
                    {mockCustomers.sort((a,b) => b.balance - a.balance).slice(0,5).map(c => (
